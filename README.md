...
SemVer `router-vMAJOR.MINOR.PATCH` releases (the `router-v*` tag line) also
provide checksum-verified native Linux archives for `x86_64` and `aarch64`.
These contain the same independent Router process used by the Omarchy
integration; installing an archive does not create or start a service or alter
