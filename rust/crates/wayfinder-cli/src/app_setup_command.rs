...
fn calibrated_automatic_preset(name: &str, base: &str) -> Result<String, String> {
    ...
    let mut corpus_sha256 = String::with_capacity(digest.len() * 2);
    for byte in digest {
        corpus_sha256.push(char::from(LOWER_HEX[usize::from(byte >> 4)]));
        corpus_sha256.push(char::from(LOWER_HEX[usize::from(byte & 0x0f)]));
    }
