...
Dispositions were added after the polish pass that closed them. `fixed@SHA`
names the commit; Tier-2/3 confirmation for anything only a device or Xcode can
show is enumerated in `docs/mobile-fidelity.md` and is **not** claimed here.

| ID | Sev | Surface / journey | Finding | Evidence | Phase | Disposition |
|---|---|---|---|---|---|---|
| UX-001 | P1 | Chat / retry | Retry duplicates the user turn instead of regenerating in place; persisted history misrepresents user actions | `AppModel.swift:438-456`; asserted by `AppModelTests.swift:237-263` | Now (pre-Phase 3) | fixed@852e37a — retry regenerates into the failed reply's slot; the test asserting duplication is inverted |
| UX-002 | P1 | Chat / streaming | Every delta force-scrolls to bottom; no scrolled-away detection, no scroll-to-bottom control; reading position is unrecoverable while streaming | `ChatTabView.swift:50-57`; rule at `docs/desktop-fidelity.md:64-65` | Now | fixed@7b82e66 — follow-the-tail gated on proximity, scroll-to-latest control, rule unit-tested in `TranscriptScrollStateTests` |
| UX-003 | P1 | Accessibility | Assistant messages carry no speaker identification and streaming has no start/finish announcements for VoiceOver | `ChatTabView.swift:291` vs `:225-227` | Now | fixed@7b82e66 — assistant turns labelled \"Wayfinder\"; lifecycle announced from the shared feedback vocabulary. Screen-off VoiceOver run is Tier 3 |
| UX-004 | P1 | Composer | "+", privacy, and send controls are 32–34 pt, below the 44 pt HIG minimum | `ChatTabView.swift:404, 427, 438` | Now | fixed@7b82e66, 6080b65 — every composer control at the 44 pt floor via `WayfinderMetrics.minimumHitTarget` |
| UX-005 | P1 | Accessibility | Fixed-frame chrome (controls, 56 pt header, 30 pt mark) has no Dynamic Type scaling; contract requires send/privacy/navigation to survive AX sizes — verify on device | `ChatTabView.swift:162, 404, 427, 438`; `RootView.swift:273`; WF-DESIGN-0020 accessibility rules | Now | fixed@7b82e66, 6080b65 — fixed frames replaced with `@ScaledMetric`; the AX5 sweep of every screen is Tier 2 |
| UX-006 | P2 | Chat / rendering | No markdown, code block, list, link, or table rendering; assistant output is plain `Text`. Becomes P1 the day a live provider ships | `ChatTabView.swift:250` | Phase 3 | fixed@73def60 — block markdown on `AttributedString`, no package |
| UX-007 | P2 | Chat / actions | No copy, share, or context menu on messages; user bubbles not even selectable | `ChatTabView.swift:211-228, 250-251` | Phase 3 | fixed@7b82e66 — copy/share context menu and accessibility actions on both roles |
| UX-008 | P2 | Composer / navigation | Text field disabled while streaming; new chat and thread switching also locked during generation with no feedback | `ChatTabView.swift:396, 127`; `AppModel.swift:462-479` | Phase 3 | fixed@852e37a, 7b82e66 — composer never disabled; navigation free during generation |
| UX-009 | P2 | Chat / title | "Wayfinder ⌄" title menu contains only a permanently disabled item plus static text — a dead affordance where the contract wants the Automatic mode exposed | `ChatTabView.swift:99-116` | Now | fixed@7b82e66 — the title menu explains Automatic routing instead of offering a disabled item |
| UX-010 | P2 | Receipts | Receipt chip copy "Routed to …" diverges from the contract's prescribed "Ran …" receipt grammar; weaker execution-boundary language | `ChatTabView.swift:262`; WF-DESIGN-0020; WF-ROADMAP-0016 | Now | fixed@7b82e66 — \"Ran …\" grammar, backed by a persisted boundary |
| UX-011 | P2 | Design system | Accent #15AB75 at ≈2.9:1 on white tints the receipt-chip icon and caption-weight badge text; no dark/increased-contrast variant possible without an asset catalog | `WayfinderTheme.swift:4-8`; `ChatTabView.swift:261`; `APIKeysView.swift:97-98` | Phase 6 | fixed@fd1df17, 6561196 — designed light/dark/increased-contrast variants; ratios asserted in `WayfinderThemeContrastTests` |
...
|---|---|---|---|---|---|
| UX-028 | P0 | Routing | **Automatic routing can never select a destination.** Every shipping destination is built `automaticEligible: false`, the routing core hard-excludes on exactly that flag, so `plan_automatic_route` returns no selection and any unpinned turn ends as a failed reply. Three separate strings told the user the opposite | `AppModel.swift` (`liveDirectProviders`, discovered models); `rust/crates/wayfinder-routing-core/src/lib.rs` `automatic_eligible` exclusion | **fixed.** Resolved as a product decision, since the routing plumbing was already complete — the configured tiers (`local` ≥ 0.0, `cloud` ≥ 0.1) already matched the destinations' `routeTier` values, and scoring, exclusions, fallbacks and receipts all worked. Only the flag was blocking. **On-device execution is now enrolled in Automatic by construction**: it sends nothing anywhere, which makes it the one case where a default costs the user nothing to consent to. **Hosted destinations join only by an explicit, reversible opt-in** in Destinations, which satisfies WF-ROADMAP-0016's rule that connecting a provider must never *silently* change routing — an explicit act is the opposite of silent. Consent persists in the workspace record and is guarded against a pre-restore save wiping it. Eligibility is now decided in exactly one place and additively, so a destination compiled eligible is never silently revoked. Copy still branches on `hasAutomaticDestination` rather than asserting an outcome. Six tests cover enrolment, withdrawal, persistence across relaunch, the early-save guard, and that saving a key still enrols nothing. Predates the branch (`b9b65ed`, `d1fa3cc`, `274d35e`) |
